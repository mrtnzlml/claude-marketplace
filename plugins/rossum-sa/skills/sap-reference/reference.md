...
- Rarely used by customers (only 1 known: Atotech)
...
|----------|-------------------|-------------|--------|
| **Atotech** | HTTPS XML API via CPI | Synced by customer via MDH API | Rossum pushes sales order to CPI |
| **allnex** | SFTP-based | Pulled from same SFTP | IDOCs generated and placed on SFTP |
| **IAG** | Hybrid | Scheduled Imports extension | Custom serverless function calling VIM Invoice API |
| **Panasonic** | SFTP-based | Pulled from same SFTP | IDOCs generated and placed on SFTP |
| **Zimmer-Biomet** | Azure API Manager | Customer's Azure middleware calls MDH API | Rossum generates IDOC XML, pushes via Azure APIM |
